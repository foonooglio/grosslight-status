...
const PUBLIC_ERROR_MESSAGES = {
  status_data_invalid: 'Status data failed validation.',
  status_data_unavailable: 'Unable to load local status data.',
...
function buildErrorResponse(error, generatedAt = isoNow()) {
  const isInvalid = error && error.code === 'STATUS_DATA_INVALID';
  const errorCode = isInvalid ? 'status_data_invalid' : 'status_data_unavailable';
  return {
    contractVersion: CONTRACT_VERSION,
    error: errorCode,
...
function buildServerErrorResponse(generatedAt = isoNow()) {
  return {
    contractVersion: CONTRACT_VERSION,
    error: 'server_error',
...
function buildNotFoundResponse(generatedAt = isoNow()) {
  return {
    contractVersion: CONTRACT_VERSION,
    error: 'not_found',
...
function buildMethodNotAllowedResponse(generatedAt = isoNow()) {
  return {
    contractVersion: CONTRACT_VERSION,
